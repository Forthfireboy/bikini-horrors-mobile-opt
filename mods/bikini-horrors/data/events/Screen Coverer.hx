...
function postUpdate(elapsed:Float) {
    if (screenCoverer != null && screenCoverer.visible) {
        if (screenCoverer.cameras != null && screenCoverer.cameras.contains(camGame)) {
            screenCoverer.angle = -camGame.angle;
        } else {
            if (screenCoverer.angle != 0) screenCoverer.angle = 0;
        }
    }
}