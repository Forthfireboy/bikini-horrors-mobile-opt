public var finalNotesScale:Float = 0.8;
static var noteSkin:String = "carma_noteskin";
static var splashSkin:String = null;

function create() {

	noteSkin = "carma_noteskin"; splashSkin = null;
...
function onStrumCreation(event) {
	...
	if (Assets.exists(Paths.image(event.sprite + "_END"))) {
		...
		strum.loadGraphic(Paths.image(event.sprite), true, 32, 32);
		strum.animation.add("static", [event.strumID]);
		strum.animation.add("pressed", [4 + event.strumID, 8 + event.strumID], 14, false);
...
function onNoteCreation(e) {
	if (e.noteType != null && Assets.exists(Paths.image("game/notes/" + e.noteType)))
		...
		if (__usePixel) {
			...
			if (e.note.isSustainNote) {
				note.loadGraphic(Paths.image(e.noteSprite + "_END"), true, 8, 8);
				note.animation.add("hold", [e.strumID]);
				note.animation.add("holdend", [4 + e.strumID]);
			} else {
				note.loadGraphic(Paths.image(e.noteSprite), true, 32, 32);
