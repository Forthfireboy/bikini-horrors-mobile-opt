...
class MusicBeatSubstate extends FlxSubState implements IBeatCancellableReceiver
{
	...
	public function addExtraHitboxKey(key:String = "SPACE", ?buttonID:String):Dynamic {
		return mobileManager.addExtraHitboxKey(key, buttonID);
	}
	public function mobileHitboxJustPressed(buttons:Dynamic):Bool {
		return mobileManager?.hitbox?.justPressed(buttons) == true;
	}
	public function mobileHitboxPressed(buttons:Dynamic):Bool {
		return mobileManager?.hitbox?.pressed(buttons) == true;
	}
	public function mobileSpaceJustPressed():Bool {
		return FlxG.keys.justPressed.SPACE || mobileHitboxJustPressed(["SPACE", "EXTRA_1"]);
	}
	...
	public function addJoyStick(x:Float = 0, y:Float = 0, ?graphic:String, ?onMove:Float->Float->Float->String->Void, size:Float = 1):Void {
		mobileManager.addJoyStick(x, y, graphic, onMove, size);
	}
	...
	public function addJoyStickCamera(defaultDrawTarget:Bool = false):Void {
		mobileManager.addJoyStickCamera(defaultDrawTarget);
	}
	public function joyStickPressed(?pos:String):Bool {
		return mobileManager?.joyStick?.pressed(pos) == true;
	}
