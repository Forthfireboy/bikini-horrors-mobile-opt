...
class MainState extends FlxState {
	...
	var __nextStateFactory:Void->FlxState = null;
	var __shaderStatusText:FlxText;
	var __shaderNames:Array<String> = [];
	var __shaderIndex:Int = 0;
	var __shaderStartTime:Float = 0;
	static inline var SHADER_TEXT_PRELOAD_BUDGET:Int = 16;
	public override function create() {
		...
		if (Options.devMode && Options.allowConfigWarning && !isZipMod && !isEmbeddedMod) {
				...
				&& lib.modName == ModsFolder.currentModFolder)
			{
				...
				var warningLib = lib;
				var warningState = startState;
				__nextStateFactory = function() {
					return new ModConfigWarning(warningLib, warningState);
				};
				break;
			}
		}

		if (__nextStateFactory == null) {
			var defaultStartState = startState;
			__nextStateFactory = function() {
				return cast Type.createInstance(defaultStartState, []);
			};
		}
	...
	override public function update(elapsed:Float) {
		...
		if (__shaderNames.length <= 0 || __shaderIndex >= __shaderNames.length) {
			switchToPreparedState();
			return;
		}

		var budget = SHADER_TEXT_PRELOAD_BUDGET;
		while (budget-- > 0 && __shaderIndex < __shaderNames.length)
			primeCurrentShader();

		if (__shaderIndex >= __shaderNames.length) {
			Logs.infos('Startup shader source preload finished in ${Std.int((Timer.stamp() - __shaderStartTime) * 1000)}ms', "MainState");
			switchToPreparedState();
		}
	}
	...
	function startShaderWarmup() {
		__shaderNames = collectShaderNames();
		if (__shaderNames.length <= 0)
			return;

		__shaderStartTime = Timer.stamp();

		__shaderStatusText = new FlxText(0, FlxG.height - 36, FlxG.width, "");
		__shaderStatusText.setFormat(Paths.font(Flags.DEFAULT_FONT), 14, FlxColor.WHITE, CENTER);
		add(__shaderStatusText);
	}

	function collectShaderNames():Array<String> {
		...
		for (asset in Assets.list()) {
			...
			if (!cleanAsset.startsWith("assets/shaders/"))
				continue;
			if (!(cleanAsset.endsWith(".frag") || cleanAsset.endsWith(".vert")))
				continue;

			addShaderName(shaderNames, cleanAsset.substr("assets/shaders/".length));
		}

		#if MOD_SUPPORT
		if (Paths.assetsTree != null) {
			for (file in Paths.assetsTree.getFiles("assets/shaders/")) {
				if (file.endsWith(".frag") || file.endsWith(".vert"))
					addShaderName(shaderNames, file);
			}
		}
		#end

		shaderNames.sort(Reflect.compare);
		return shaderNames;
	}

	function addShaderName(shaderNames:Array<String>, file:String) {
		var shaderName = file.substr(0, file.length - 5);
		shaderNames.pushOnce(shaderName);
	}

	function primeCurrentShader() {
		var shaderName = __shaderNames[__shaderIndex];
		try {
			CustomShader.preload(shaderName);
		} catch (e:Dynamic) {
			Logs.warn('Failed to preload shader source "$shaderName": ${Std.string(e)}', "MainState");
		}
		if (__shaderStatusText != null)
			__shaderStatusText.text = 'Loading shaders ${__shaderIndex + 1}/${__shaderNames.length}: $shaderName';
		__shaderIndex++;
	}

	function switchToPreparedState() {
		if (__nextStateFactory == null)
			return;

		var nextState = __nextStateFactory;
		__nextStateFactory = null;
