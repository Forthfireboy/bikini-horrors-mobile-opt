...
class MobileUtil
{
	...
	public static inline function getStorageTypePath():String
		return AndroidContext.getFilesDir() + '/storagetype.txt';
	...
	public static function initDirectory():String {
		var daPath:String = Path.addTrailingSlash(AndroidContext.getFilesDir());
		...
		{
			trace('Could not create private app directory $daPath: $e');
		}
		...
		{
			trace('Could not create private mods directory ${MobileUtil.getDirectory()}mods/: $e');
		}
	...
	{
		// APK-only mode reads bundled assets and writes only to the app-private directory.
		// No storage or media permissions are required.
	}
